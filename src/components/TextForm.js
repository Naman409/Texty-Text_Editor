    ...
    return (
        <>
            ...
            <<div className="container" style={{ color: props.mode === 'dark' ? 'white' : 'black' }} >
                ...
                <p>Number of Words : <b>{text.split(/\s+/).filter((element)=>{return element.length!==0}).length}</b> and Number of Characters : <b>{text.length}</b></p>
                <p>Time to read : <b> {0.008 * (text.split(" ").length)} </b> minutes</p>
                <h2>Preview</h2>
                <p>{text}</p>
